...
func (o *ClusterUninstaller) listCloudConnections() (cloudResources, error) {
	...
	for _, cloudConnection = range cloudConnections.CloudConnections {
		...
		for !o.timeout(ctx) {
			...
			EndpointVpc = cloudConnection.Vpc
			o.Logger.Printf("listCloudConnections: EndpointVpc = %+v", EndpointVpc)
			...
			for _, Vpc = range EndpointVpc.Vpcs {
				...
				o.Logger.Printf("listCloudConnections: Vpc = %+v", Vpc)
			}
			o.Logger.Printf("listCloudConnections: foundVpc = %v", foundVpc)
			if foundVpc {
		...
		o.Logger.Printf("listCloudConnections: jobReference.ID = %s", *jobReference.ID)
...
func (o *ClusterUninstaller) listDHCPNetworks() (cloudResources, error) {
	...
	for _, dhcpServer = range dhcpServers {
		...
		if dhcpServer.Network.Name == nil {
			...
			if shouldDeleteDHCP {
				o.Logger.Debugf("listDHCPNetworks: deleting DHCP id %v because shouldDeleteDHCP", *dhcpServer.ID)
				item := cloudResource{
					key:      *dhcpServer.ID,
					name:     "",
					status:   "",
				...
				if err != nil {
					return nil, errors.Wrapf(err, "failed to delete DHCP id %v and the response is: %s", *dhcpServer.ID)
				}
			}
			continue
		}
...
func (o *ClusterUninstaller) listDNSRecords() (cloudResources, error) {
	...
	select {
	case <-o.Context.Done():
		o.Logger.Debugf("listDNSRecords: case <-o.Context.Done()")
		return nil, o.Context.Err() // we're cancelled, abort
	default:
	}
...
func (o *ClusterUninstaller) destroyDNSRecords() error {
	if o.dnsRecordsSvc == nil {
		return errors.Errorf("destroyDNSRecords: o.dnsRecordsSvc is nil!")
	}
...
func (o *ClusterUninstaller) listResourceRecords() (cloudResources, error) {
	...
	select {
	case <-o.Context.Done():
		o.Logger.Debugf("listResourceRecords: case <-o.Context.Done()")
		return nil, o.Context.Err() // we're cancelled, abort
	default:
	}
...
func (o *ClusterUninstaller) destroyResourceRecord(item cloudResource) error {
	...
	select {
	case <-o.Context.Done():
		o.Logger.Debugf("destroyResourceRecord: case <-o.Context.Done()")
		return o.Context.Err() // we're cancelled, abort
	default:
	}
	...
	o.Logger.Debugf("Deleting DNS Resource record %q", item.name)
	o.deletePendingItems(item.typeName, []cloudResource{item})
...
func (o *ClusterUninstaller) destroyResourceRecords() error {
	if o.resourceRecordsSvc == nil {
		return errors.Errorf("destroyResourceRecords: o.resourceRecordsSvc is nil!")
	}
	...
	for _, item := range items {
		select {
		case <-o.Context.Done():
			o.Logger.Debugf("destroyResourceRecords: case <-o.Context.Done()")
			return o.Context.Err() // we're cancelled, abort
		default:
		}
...
func (o *ClusterUninstaller) deleteVPC(item cloudResource) error {
	...
	o.Logger.Printf("deleteVPC: getResponse = %v", getResponse)
	o.Logger.Printf("deleteVPC: err = %v", err)
...
func (o *ClusterUninstaller) loadSDKServices() error {
	...
	userAgentString := fmt.Sprintf("OpenShift/4.x Destroyer/%s", "@TODO")
