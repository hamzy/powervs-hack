...
func listInstances (rSearch *regexp.Regexp, piInstanceClient *instance.IBMPIInstanceClient, serviceGuid string) {

	var err error

	instances, err := piInstanceClient.GetAll(serviceGuid, 60*time.Minute)
	if err != nil {
...
func main() {
	...
	log.Printf("piInstanceClient = %v\n", piInstanceClient)

	listInstances(rSearch, piInstanceClient, serviceGuid)
}
