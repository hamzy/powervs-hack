...
func cleanupSSHKey (sshKey string, piSession *ibmpisession.IBMPISession, serviceGuid string) {

	var tenantId = piSession.UserAccount
	var err error

	params := p_cloud_tenants_ssh_keys.NewPcloudTenantsSshkeysDeleteParamsWithTimeout(helpers.PIDeleteTimeOut).WithTenantID(tenantId).WithSshkeyName(sshKey)
...
func listSSHKeys (rSearch *regexp.Regexp, piSession *ibmpisession.IBMPISession, serviceGuid string) {
	...
	for _, sshKey := range sshKeys.SSHKeys {
		if rSearch.MatchString(*sshKey.Name) {
			log.Printf("sshKey.Name = %v\n", *sshKey.Name)
			log.Printf("sshKey.SSHKey = %v\n", *sshKey.SSHKey)

			//cleanupSSHKey (*sshKey.Name, piSession, serviceGuid)
		}
	}
...
func main() {
	...
	listSSHKeys(rSearch, piSession, serviceGuid)
}
